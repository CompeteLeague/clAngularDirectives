 ...
 * License: MIT
 */
!function(){"use strict";var t=angular.module("cl.table",[]);t.directive("clTable",function(){return{templateUrl:"clTable.html",restrict:"E",scope:{options:"=options"},link:function(t,e,n){t.searchItem={},t.sortType=t.options.contentAttributes[0],t.sortReverse=!1,t.sort=function(e){t.sortType=t.options.contentAttributes[e],t.sortReverse=!t.sortReverse},t.isObject=function(t){return angular.isObject(t)}}}})}(),angular.module("cl.table").run(["$templateCache",function(t){t.put("clTable.html",'<div ng-if="options.searchable" ng-init="showFilter=false"><button class="btn btn-primary" ng-click="showFilter=!showFilter"><span ng-show="!showFilter" class="glyphicon glyphicon-eye-open"></span> <span ng-show="showFilter" class="glyphicon glyphicon-eye-close"></span> Filter</button><br><div class="form-group" ng-repeat="item in options.contentAttributes" ng-show="showFilter"><div ng-hide="isObject(options.content[0][item])"><label>{{options.heading[$index]}}</label> <input type="text" class="form-control" ng-model="searchItem[item]"></div></div></div><table class="table"><tr><th ng-repeat="headline in options.heading" ng-click="sort($index)">{{headline}}</th></tr><tr ng-repeat="item in options.content | orderBy:sortType:sortReverse | filter:searchItem"><td ng-repeat="attribute in options.contentAttributes"><span ng-if="!isObject(item[attribute])">{{item[attribute]}}</span> <button ng-if="isObject(item[attribute])" ng-click="item[attribute].fn(item)" class="btn btn-sm" ng-class="item[attribute].class">{{item[attribute].display}}</button></td></tr></table>')}]);